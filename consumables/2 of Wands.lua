...
SMODS.Consumable {
    ...
    loc_txt = {
        name = '2 of Wands',
        text = {
            [1] = 'Adds {C:blue}+5{} Chips for every {C:attention}2{} is your Deck to {C:attention}1{} selected card.',
        ...
        if to_big(#G.hand.highlighted) == to_big(1) then
            ...
            for i = 1, #G.hand.highlighted do
                G.E_MANAGER:add_event(Event({
                    ...
                    G.hand.highlighted[i].ability.perma_bonus = G.hand.highlighted[i].ability.perma_bonus + ((function() local count = 0; for _, card in ipairs(G.playing_cards or {}) do if card.base.id == 2 then count = count + 1 end end; return count end)()) * 5
                        return true
                    end
                }))
            end
